...
class MeetingService:
    ...
    def list_meetings(
        ...
        logger.info(
            "Listing meetings owner=%s status=%s limit=%s offset=%s sort_by=%s order=%s",
            owner,
            status,
            limit,
            offset,
        ...
        if sort_by not in {"date", "title", "owner"}:
        ...
        if owner:
            items = [
                (meeting_id, meeting)
                for meeting_id, meeting in items
        ...
        if sort_by == "date":
            ...
            items = sorted(
                items,
                key=lambda item: (item[1].owner.lower(), item[0]),
                reverse=reverse,
            )
        ...
        logger.info("Found %s meetings after filters", total)
    ...
    def add_action_item(
        ...
        logger.info(
            "Action item added to meeting id=%s action_item_id=%s",
            meeting_id,
            item.id,
        )
        return item
